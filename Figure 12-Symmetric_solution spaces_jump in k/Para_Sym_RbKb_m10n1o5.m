...
A2bar= 1;
k2=1.1;
m=10;% Go to classical_Sym_gen.m and choose the appropriate initial guess for the classical case
n=1.5;
...
guess_Ro0oS = [0.0507    0.0859    0.1846    0.3380]; 
guess_Ro0oR = [0.0507    0.0859    0.1846    0.3380];
guess_RoSS = [0.0065    0.0200 ]; 
guess_RoSR = [0.0065    0.0200];


% Find the solutions for different u2 and A2, and plot them 
% First set the ranges of u2 and A2
dur1 = 0.025;
u2range =-2.5:dur1:2.5;
...
if u2<c2 && u2>-c2  % Consider only subcritical regime
   ...
   [x, exitflag] = classical_Sym_gen(u1,u2,k2,m,A2bar,A1,A2,n);
    ...
    [exitflag_RoSS,guess_RoSS] = resonance_RoSS_Sym_gen(u1,u2,k2,m,n,A2bar,A1,A2,guess_RoSS,A12_guess);
    [exitflag_RoSR,guess_RoSR] = resonance_RoSR_Sym_gen(u1,u2,k2,m,n,A2bar,A1,A2,guess_RoSR,A12_guess);
    [exitflag_Ro0oS,guess_Ro0oS] = resonance_Ro0oS_Sym_gen(u1,u2,k2,m,n,A2bar,A1,A2,guess_Ro0oS,A11_guess);
    [exitflag_Ro0oR,guess_Ro0oR] = resonance_Ro0oR_Sym_gen(u1,u2,k2,m,n,A2bar,A1,A2,guess_Ro0oR,A11_guess);
        ...
        elseif exitflag_Ro0oS == 1 && guess_Ro0oS(1)>0 && guess_Ro0oS(2)>0 && guess_Ro0oS(3)>0 && guess_Ro0oS(4)>0 && guess_Ro0oS(5)
            hold on
...
% Critical boundary
c2=sqrt(k2*(m*(A2range/A2bar).^m+n*(A2range/A2bar).^(-n)));
hold on
plot(A2range,c2,'k')
ylim([-2.5 2.5])
plot(A2range,-c2,'k')