...
A11_guess = 1.05;
A12_guess = A11_guess;
guess_Ro0oS = [0.5937    0.0979    0.1920    0.2169];
guess_Ro0oR = [0.2759    0.1127    0.1835    0.2318];
guess_RoSS = [0.0855    0.1189 ];
guess_RoSR = [0.0979    0.1920];
...
for un = 1:length(u2range)
 ...
 if u2<c2 && u2>-c2  % Consider only subcritical regime
    ...
    [x, exitflag] = classical_sing_gen(u1,u2,k2,m1,m2,A2bar,A1,A2,n1,n2);
  ...
  figure(1)
       if  exitflag==1 && x(3)>0 && x(1)>0 
        ...
        [exitflag_RoSS,guess_RoSS] = resonance_RoSS_sing_gen(u1,u2,k2,m1,m2,n1,n2,A2bar,A1,A2,guess_RoSS,A12_guess);
        [exitflag_RoSR,guess_RoSR] = resonance_RoSR_sing_gen(u1,u2,k2,m1,m2,n1,n2,A2bar,A1,A2,guess_RoSR,A12_guess);
        [exitflag_Ro0oS,guess_Ro0oS] = resonance_Ro0oS_sing_gen(u1,u2,k2,m1,m2,n1,n2,A2bar,A1,A2,guess_Ro0oS,A11_guess);
        [exitflag_Ro0oR,guess_Ro0oR] = resonance_Ro0oR_sing_gen(u1,u2,k2,m1,m2,n1,n2,A2bar,A1,A2,guess_Ro0oR,A11_guess);
