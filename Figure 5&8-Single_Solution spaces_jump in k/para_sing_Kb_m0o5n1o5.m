...
A2bar= 1;
k2=1.1;
m=0.5; % Go to classical_sing_gen.m and choose the appropriate initial guess for the classical case
n=1.5;

% Initial guesses for resonant cases
A11_guess = 1.05;
A12_guess = A11_guess;
guess_Ro0oS = [0.10882    0.1005    0.6871    0.7132];
guess_Ro0oR = [0.2587    0.3765    0.4959    0.5630];
guess_RoSS = [0.0947    0.6887];
guess_RoSR = [0.2741    0.4020];

A22_guess=1.3;
A21_guess=A22_guess;
guess_So0oR = [0.0708    1.8801    1.5498    1.2231];
guess_SSoR = [2.1599    1.2124];
 ...
 if u2<c2 && u2>-c2  % Consider only subcritical regime
  ...
  figure(1)
     ...
     if  exitflag==1 && x(3)>0 && x(1)>0 
        ...
        [exitflag_So0oR,guess_So0oR] = resonance_So0oR_sing_gen(u1,u2,k2,m,n,A2bar,A1,A2,guess_So0oR,A22_guess);
        [exitflag_SSoR,guess_SSoR] = resonance_SSoR_sing_gen(u1,u2,k2,m,n,A2bar,A1,A2,guess_SSoR,A21_guess);
